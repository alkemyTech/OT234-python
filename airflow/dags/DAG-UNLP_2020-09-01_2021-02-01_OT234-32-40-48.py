...
fh = logging.FileHandler(FILES_PATH.joinpath('DAG-UNLP_2020-09-01_2021-02-01_OT234-32-40-48.log'))
...
def extract_from_db():
    ...
    It saves data extracted in FILES_PATH as raw_data.csv.
    The values used are:
        CONN_ID = 'alkemy_db'
        TABLE = 'training'
        SQL_PATH = 'include/'
    ...
    with open(SQL_PATH.joinpath('UNLP_2020-09-01_2021-02-01_OT234-16.sql'), 'r') as file:
        sql = file.read()
    ...
    pd.read_sql(sql, con=conn).to_csv(FILES_PATH.joinpath('UNLP_raw_data.csv'))
...
def transform_extrated_data():
    ...
    """
    raw_data = pd.read_csv(FILES_PATH.joinpath('UNLP_raw_data.csv'), index_col='Unnamed: 0')
    dataset = tf.transform_OT234_72(raw_data)
    try:
        dataset.to_csv(PROCESSED_DATA_PATH.joinpath('UNLP_dataset.csv'))
...
def load_to_s3():
    ...
    """
    dataset = pd.read_csv(PROCESSED_DATA_PATH.joinpath('UNLP_dataset.csv'), index_col='Unnamed: 0')
