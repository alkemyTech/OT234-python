...
def Process_UNJu(Univ, raw_path, datasets_path):
    # Path to .csv files
    ...
    for rws in range(0, len(DRaw)):
        university = 'universidad nacional de jujuy'   # University
        career = re.sub('_',' ',DRaw.career[rws])
        inscription_date = DRaw.inscription_date[rws] 

        full_name = DRaw.nombre[rws].split(' ')  # Names
        if len(full_name) == 2 :     #          
        ...
        BDate = parser.parse(DRaw.birth_date[rws])  
        ...
        Addr_String = DRaw.direccion[rws].splitlines()[1]    # Address string splitting
        try: 
